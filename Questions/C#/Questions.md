...
## Оглавление
    ...
    - [2. **Метод `.Equals()`**](#2-метод-equals)
    - [**Когда что использовать?**](#когда-что-использовать)
    - [**Пример с пользовательским классом**](#пример-с-пользовательским-классом)
  - [<b style="color:yellowgreen;">Разница между **const** и **readonly**</b>](#b-stylecoloryellowgreenразница-между-const-и-readonlyb)
    - [**1. `const`**](#1-const)
    - [**2. `readonly`**](#2-readonly)
    - [**Ключевые различия**](#ключевые-различия)
    - [**Когда что использовать?**](#когда-что-использовать)
- [<b style="color:yellowgreen;">Что такое delegate</b>](#b-stylecoloryellowgreenчто-такое-delegateb)
    - [**1. Объявление делегата**](#1-объявление-делегата)
    - [**2. Создание метода, соответствующего делегату**](#2-создание-метода-соответствующего-делегату)
    - [**3. Присвоение метода делегату**](#3-присвоение-метода-делегату)
    - [**4. Использование анонимных методов и лямбда-выражений**](#4-использование-анонимных-методов-и-лямбда-выражений)
      - [**Анонимный метод:**](#анонимный-метод)
      - [**Лямбда-выражение:**](#лямбда-выражение)
    - [**5. Мультикастинг делегатов**](#5-мультикастинг-делегатов)
    - [**6. Пример с делегатом `Action` и `Func`**](#6-пример-с-делегатом-action-и-func)
      - [**`Action`:**](#action)
      - [**`Func`:**](#func)
    - [**Полный пример:**](#полный-пример)
- [<b style="color:yellowgreen;">Какие есть ограничения у delegate и является ли он потокобезопасным</b>](#b-stylecoloryellowgreenкакие-есть-ограничения-у-delegate-и-является-ли-он-потокобезопаснымb)
  - [**Ограничения делегатов**](#ограничения-делегатов)
    - [1. **Типизация и сигнатура**](#1-типизация-и-сигнатура)
    - [2. **Мультикастинг и возвращаемые значения**](#2-мультикастинг-и-возвращаемые-значения)
    - [3. **Исключения в мультикастинге**](#3-исключения-в-мультикастинге)
    - [4. **Производительность**](#4-производительность)
  - [**Потокобезопасность делегатов**](#потокобезопасность-делегатов)
    - [1. **Вызов делегатов**](#1-вызов-делегатов)
    - [2. **Мультикастинг и потокобезопасность**](#2-мультикастинг-и-потокобезопасность)
    - [3. **Как обеспечить потокобезопасность?**](#3-как-обеспечить-потокобезопасность)
  - [**Когда делегаты потокобезопасны?**](#когда-делегаты-потокобезопасны)
  - [**Рекомендации**](#рекомендации)
    - [**Пример потокобезопасного изменения делегата**](#пример-потокобезопасного-изменения-делегата)
- [<b style="color:yellowgreen;">Разница между конвертацией as и ()</b>](#b-stylecoloryellowgreenразница-между-конвертацией-as-и-b)
  - [**1. Оператор `()` (явное приведение)**](#1-оператор-явное-приведение)
  - [**2. Оператор `as`**](#2-оператор-as)
  - [**Ключевые различия**](#ключевые-различия)
  - [**Когда что использовать?**](#когда-что-использовать)
  - [**Пример комбинированного использования**](#пример-комбинированного-использования)
  - [**Важно!**](#важно)
- [<b style="color:yellowgreen;">Что такое LINQ кратко и с примерами</b>](#b-stylecoloryellowgreenчто-такое-linq-кратко-и-с-примерамиb)
    - [**Основные возможности LINQ**](#основные-возможности-linq)
    - [**Синтаксис LINQ**](#синтаксис-linq)
    - [**Примеры использования LINQ**](#примеры-использования-linq)
      - [**1. Фильтрация (`Where`)**](#1-фильтрация-where)
      - [**2. Проекция (`Select`)**](#2-проекция-select)
      - [**3. Сортировка (`OrderBy`, `OrderByDescending`)**](#3-сортировка-orderby-orderbydescending)
      - [**4. Группировка (`GroupBy`)**](#4-группировка-groupby)
      - [**5. Агрегация (`Count`, `Sum`, `Average`)**](#5-агрегация-count-sum-average)
      - [**6. Объединение (`Join`)**](#6-объединение-join)
    - [**Преимущества LINQ**](#преимущества-linq)
    - [**Пример с отложенным выполнением**](#пример-с-отложенным-выполнением)
    - [**Когда использовать LINQ?**](#когда-использовать-linq)
- [<b style="color:yellowgreen;">Что изпользует LINQ для своих методов, опиши delegate и patterns которые он использует</b>](#b-stylecoloryellowgreenчто-изпользует-linq-для-своих-методов-опиши-delegate-и-patterns-которые-он-используетb)
  - [**1. Делегаты и лямбда-выражения в LINQ**](#1-делегаты-и-лямбда-выражения-в-linq)
    - [**Основные делегаты в LINQ:**](#основные-делегаты-в-linq)
    - [**Как LINQ использует делегаты?**](#как-linq-использует-делегаты)
      - [**Пример с `Where`:**](#пример-с-where)
      - [**Эквивалент без LINQ:**](#эквивалент-без-linq)
  - [**2. Методы расширения (Extension Methods)**](#2-методы-расширения-extension-methods)
    - [**Как работают методы расширения?**](#как-работают-методы-расширения)
      - [**Пример метода расширения:**](#пример-метода-расширения)
  - [**3. Шаблоны проектирования в LINQ**](#3-шаблоны-проектирования-в-linq)
    - [**а) Шаблон "Итератор" (Iterator Pattern)**](#а-шаблон-итератор-iterator-pattern)
      - [**Пример отложенного выполнения:**](#пример-отложенного-выполнения)
    - [**б) Шаблон "Цепочка обязанностей" (Chain of Responsibility)**](#б-шаблон-цепочка-обязанностей-chain-of-responsibility)
    - [**в) Шаблон "Стратегия" (Strategy Pattern)**](#в-шаблон-стратегия-strategy-pattern)
  - [**4. Пример реализации простого LINQ-метода**](#4-пример-реализации-простого-linq-метода)
  - [**5. Как LINQ работает с базами данных?**](#5-как-linq-работает-с-базами-данных)
      - [**Пример с Entity Framework:**](#пример-с-entity-framework)
  - [**Итоги**](#итоги)
- [<b style="color:yellowgreen;">Func<T>, Action<T>, Predicate<T> Опиши данные delegate, сценарии их использования, какие PATTERNS используют</b>](#b-stylecoloryellowgreenfunct-actiont-predicatet-опиши-данные-delegate-сценарии-их-использования-какие-patterns-используютb)
    - [**Сравнительная таблица делегатов**](#сравнительная-таблица-делегатов)
    - [**Примеры с пояснениями**](#примеры-с-пояснениями)
      - [**1. `Func<T>` (Strategy Pattern)**](#1-funct-strategy-pattern)
      - [**2. `Action<T>` (Observer и Command Patterns)**](#2-actiont-observer-и-command-patterns)
      - [**3. `Predicate<T>` (Specification Pattern)**](#3-predicatet-specification-pattern)
    - [**Итоги**](#итоги)
  - [**1. `virtual`**](#1-virtual)
  ...
  - [**Полиморфизм на практике**](#полиморфизм-на-практике)
  - [**Важные моменты**](#важные-моменты)
  - [**Когда что использовать?**](#когда-что-использовать)
  - [**1. Зачем нужны Generics?**](#1-зачем-нужны-generics)
  - [**2. Синтаксис Generics**](#2-синтаксис-generics)
    - [**Примеры:**](#примеры)
      - [**Обобщённый класс:**](#обобщённый-класс)
      - [**Обобщённый метод:**](#обобщённый-метод)
      - [**Обобщённый интерфейс:**](#обобщённый-интерфейс)
  - [**3. Сценарии использования Generics**](#3-сценарии-использования-generics)
  - [**4. Ограничения на параметры типа (`where T : ...`)**](#4-ограничения-на-параметры-типа-where-t)
    - [**Виды ограничений:**](#виды-ограничений)
    - [**Пример с `where T : class`**](#пример-с-where-t-class)
  - [**5. Преимущества и ограничения Generics**](#5-преимущества-и-ограничения-generics)
    - [**Преимущества:**](#преимущества)
    - [**Ограничения:**](#ограничения)
  - [**6. Пример с несколькими ограничениями**](#6-пример-с-несколькими-ограничениями)
  - [**7. Когда использовать Generics?**](#7-когда-использовать-generics)
  - [**1. Значимые и ссылочные типы**](#1-значимые-и-ссылочные-типы)
  - [**2. Boxing (упаковка)**](#2-boxing-упаковка)
    - [**Как это работает?**](#как-это-работает)
    - [**Пример:**](#пример)
  - [**3. Unboxing (распаковка)**](#3-unboxing-распаковка)
    - [**Как это работает?**](#как-это-работает)
    - [**Пример:**](#пример)
  - [**4. Почему boxing/unboxing важны?**](#4-почему-boxingunboxing-важны)
  - [**5. Примеры boxing/unboxing**](#5-примеры-boxingunboxing)
    - [**Пример 1: Boxing**](#пример-1-boxing)
    - [**Пример 2: Unboxing**](#пример-2-unboxing)
    - [**Пример 3: Ошибка при unboxing**](#пример-3-ошибка-при-unboxing)
  - [**6. Когда происходит boxing/unboxing?**](#6-когда-происходит-boxingunboxing)
  - [**7. Как избежать boxing/unboxing?**](#7-как-избежать-boxingunboxing)
    - [**Пример: Обобщённые коллекции**](#пример-обобщённые-коллекции)
  - [**8. Производительность**](#8-производительность)
    - [**Пример: Замеры производительности**](#пример-замеры-производительности)
  - [**9. Итоги**](#9-итоги)

...
## Оглавление
- [**1. Базовые коллекции: `List<T>`, `Array`, `LinkedList<T>`**](#1-базовые-коллекции-listt-array-linkedlistt)
  - [**Вопрос 1: Чем отличается `List<T>` от массива (`T[]`)? Когда что использовать?**](#вопрос-1-чем-отличается-listt-от-массива-t-когда-что-использовать)
  - [**Вопрос 2: Как устроен `LinkedList<T>`? В каких сценариях он эффективнее `List<T>`?**](#вопрос-2-как-устроен-linkedlistt-в-каких-сценариях-он-эффективнее-listt)
- [**2. Словарные коллекции: `Dictionary<TKey, TValue>`, `Hashtable`, `SortedDictionary<TKey, TValue>`**](#2-словарные-коллекции-dictionarytkey-tvalue-hashtable-sorteddictionarytkey-tvalue)
  - [**Вопрос 3: Как работает `Dictionary<TKey, TValue>`? Что такое "корзина" (bucket)?**](#вопрос-3-как-работает-dictionarytkey-tvalue-что-такое-корзина-bucket)
  - [**Вопрос 4: Чем `Dictionary<TKey, TValue>` отличается от `Hashtable`?**](#вопрос-4-чем-dictionarytkey-tvalue-отличается-от-hashtable)
  - [**Вопрос 5: Как работает `SortedDictionary<TKey, TValue>`? В чём его отличие от `SortedList<TKey, TValue>`?**](#вопрос-5-как-работает-sorteddictionarytkey-tvalue-в-чём-его-отличие-от-sortedlisttkey-tvalue)
- [**3. Очереди и стеки: `Queue<T>`, `Stack<T>`, `ConcurrentQueue<T>`, `ConcurrentStack<T>`**](#3-очереди-и-стеки-queuet-stackt-concurrentqueuet-concurrentstackt)
  - [**Вопрос 6: Как устроен `Queue<T>`? Почему он эффективнее `List<T>` для операций FIFO?**](#вопрос-6-как-устроен-queuet-почему-он-эффективнее-listt-для-операций-fifo)
  - [**Вопрос 7: Чем `ConcurrentQueue<T>` отличается от `Queue<T>`?**](#вопрос-7-чем-concurrentqueuet-отличается-от-queuet)
- [**4. Потокобезопасные коллекции: `ConcurrentDictionary<TKey, TValue>`, `BlockingCollection<T>`**](#4-потокобезопасные-коллекции-concurrentdictionarytkey-tvalue-blockingcollectiont)
  - [**Вопрос 8: Как работает `ConcurrentDictionary<TKey, TValue>`?**](#вопрос-8-как-работает-concurrentdictionarytkey-tvalue)
  - [**Вопрос 9: Зачем нужна `BlockingCollection<T>`? Как она работает?**](#вопрос-9-зачем-нужна-blockingcollectiont-как-она-работает)
- [**5. Специализированные коллекции: `ImmutableCollections`, `Span<T>`, `Memory<T>`**](#5-специализированные-коллекции-immutablecollections-spant-memoryt)
  - [**Вопрос 10: Что такое `ImmutableDictionary<TKey, TValue>`? Когда его использовать?**](#вопрос-10-что-такое-immutabledictionarytkey-tvalue-когда-его-использовать)
  - [**Вопрос 11: Как работают `Span<T>` и `Memory<T>`? В чём их преимущества?**](#вопрос-11-как-работают-spant-и-memoryt-в-чём-их-преимущества)
  - [**Вопрос 12: Что такое `ArrayPool<T>`? Когда его использовать?**](#вопрос-12-что-такое-arraypoolt-когда-его-использовать)
- [**6. Производительность и оптимизации**](#6-производительность-и-оптимизации)
  - [**Вопрос 13: Почему `List<T>.ForEach` медленнее обычного `for`?**](#вопрос-13-почему-listtforeach-медленнее-обычного-for)
  - [**Вопрос 14: Как оптимизировать работу с `Dictionary<TKey, TValue>` для высоконагруженных сценариев?**](#вопрос-14-как-оптимизировать-работу-с-dictionarytkey-tvalue-для-высоконагруженных-сценариев)
  - [**Вопрос 15: Как выбрать коллекцию для высокопроизводительного сценария?**](#вопрос-15-как-выбрать-коллекцию-для-высокопроизводительного-сценария)
- [**1. Реализация `IEnumerable<T>` и `ICollection<T>`**](#1-реализация-ienumerablet-и-icollectiont)
  - [**Вопрос 1: Как реализовать кастомную коллекцию с поддержкой `foreach`?**](#вопрос-1-как-реализовать-кастомную-коллекцию-с-поддержкой-foreach)
  - [**Вопрос 2: Как работает `yield return` под капотом?**](#вопрос-2-как-работает-yield-return-под-капотом)
- [**2. Внутреннее устройство `List<T>`**](#2-внутреннее-устройство-listt)
  - [**Вопрос 3: Как устроен `List<T>` на уровне памяти?**](#вопрос-3-как-устроен-listt-на-уровне-памяти)
  - [**Вопрос 4: Почему `List<T>.Remove` имеет сложность `O(n)`?**](#вопрос-4-почему-listtremove-имеет-сложность-on)
- [**3. Внутреннее устройство `Dictionary<TKey, TValue>`**](#3-внутреннее-устройство-dictionarytkey-tvalue)
  - [**Вопрос 5: Как работает хэш-таблица в `Dictionary<TKey, TValue>`?**](#вопрос-5-как-работает-хэш-таблица-в-dictionarytkey-tvalue)
  - [**Вопрос 6: Почему `Dictionary<TKey, TValue>` в .NET Core 3.0+ использует деревья для коллизий?**](#вопрос-6-почему-dictionarytkey-tvalue-в-net-core-30-использует-деревья-для-коллизий)
- [**4. Внутреннее устройство `HashSet<T>`**](#4-внутреннее-устройство-hashsett)
  - [**Вопрос 7: Как устроен `HashSet<T>`? Чем он отличается от `Dictionary<TKey, TValue>`?**](#вопрос-7-как-устроен-hashsett-чем-он-отличается-от-dictionarytkey-tvalue)
  - [**Вопрос 8: Почему `HashSet<T>` не гарантирует порядок элементов?**](#вопрос-8-почему-hashsett-не-гарантирует-порядок-элементов)
- [**5. Кастомные коллекции: реализация с нуля**](#5-кастомные-коллекции-реализация-с-нуля)
  - [**Вопрос 9: Как реализовать кастомный `Dictionary` с открытой адресацией?**](#вопрос-9-как-реализовать-кастомный-dictionary-с-открытой-адресацией)
  - [**Вопрос 10: Как реализовать кастомный `HashSet<T>` с битовыми метками?**](#вопрос-10-как-реализовать-кастомный-hashsett-с-битовыми-метками)
- [**6. Продвинутые сценарии и оптимизации**](#6-продвинутые-сценарии-и-оптимизации)
  - [**Вопрос 11: Как реализовать `ICollection<T>` для кастомной коллекции с поддержкой `CopyTo`?**](#вопрос-11-как-реализовать-icollectiont-для-кастомной-коллекции-с-поддержкой-copyto)
  - [**Вопрос 12: Как реализовать кастомный итератор с поддержкой `Reset`?**](#вопрос-12-как-реализовать-кастомный-итератор-с-поддержкой-reset)
  - [**Вопрос 13: Как оптимизировать кастомную коллекцию для минимальных аллокаций?**](#вопрос-13-как-оптимизировать-кастомную-коллекцию-для-минимальных-аллокаций)
  - [**Вопрос 14: Как реализовать кастомный `Dictionary` с поддержкой `IReadOnlyDictionary<TKey, TValue>`?**](#вопрос-14-как-реализовать-кастомный-dictionary-с-поддержкой-ireadonlydictionarytkey-tvalue)
  - [**Вопрос 15: Как реализовать кастомную коллекцию с поддержкой `ICollection<T>.Remove` за `O(1)`?**](#вопрос-15-как-реализовать-кастомную-коллекцию-с-поддержкой-icollectiontremove-за-o1)
- [**1. Назначение и роль в иерархии интерфейсов**](#1-назначение-и-роль-в-иерархии-интерфейсов)
  - [**`IEnumerable<T>`**](#ienumerablet)
  - [**`ICollection<T>`**](#icollectiont)
- [**2. Методы и свойства**](#2-методы-и-свойства)
- [**3. Ключевые различия**](#3-ключевые-различия)
- [**4. Сценарии использования**](#4-сценарии-использования)
  - [**Когда использовать `IEnumerable<T>`:**](#когда-использовать-ienumerablet)
  - [**Когда использовать `ICollection<T>`:**](#когда-использовать-icollectiont)
- [**5. Примеры кода**](#5-примеры-кода)
  - [**Пример 1: Реализация `IEnumerable<T>`**](#пример-1-реализация-ienumerablet)
  - [**Пример 2: Реализация `ICollection<T>`**](#пример-2-реализация-icollectiont)
- [**6. Внутренние различия в реализации**](#6-внутренние-различия-в-реализации)
- [**7. Типичные ошибки и антипаттерны**](#7-типичные-ошибки-и-антипаттерны)
  - [**Ошибка 1: Модификация коллекции во время итерации**](#ошибка-1-модификация-коллекции-во-время-итерации)
  - [**Ошибка 2: Неэффективное использование `IEnumerable<T>`**](#ошибка-2-неэффективное-использование-ienumerablet)
  - [**Ошибка 3: Избыточная реализация `ICollection<T>`**](#ошибка-3-избыточная-реализация-icollectiont)
- [**8. Когда использовать `IReadOnlyCollection<T>`?**](#8-когда-использовать-ireadonlycollectiont)
- [**9. Сравнение с другими интерфейсами**](#9-сравнение-с-другими-интерфейсами)

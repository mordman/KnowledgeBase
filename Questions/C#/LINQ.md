...
## Оглавление
- [**1. Основы LINQ**](#1-основы-linq)
  - [**Вопрос 1: Что такое LINQ? Как он работает под капотом?**](#вопрос-1-что-такое-linq-как-он-работает-под-капотом)
  - [**Вопрос 2: Что такое отложенное выполнение (Deferred Execution) в LINQ? Приведите примеры.**](#вопрос-2-что-такое-отложенное-выполнение-deferred-execution-в-linq-приведите-примеры)
  - [**Вопрос 3: Какие операторы LINQ выполняются немедленно (Eager Evaluation)?**](#вопрос-3-какие-операторы-linq-выполняются-немедленно-eager-evaluation)
- [**2. Внутренние механизмы LINQ**](#2-внутренние-механизмы-linq)
  - [**Вопрос 4: Как работает цепочка вызовов LINQ? Что такое "итераторы" (yield)?**](#вопрос-4-как-работает-цепочка-вызовов-linq-что-такое-итераторы-yield)
  - [**Вопрос 5: Как LINQ оптимизирует запросы? Приведите примеры оптимизаций.**](#вопрос-5-как-linq-оптимизирует-запросы-приведите-примеры-оптимизаций)
  - [**Вопрос 6: Как работает `GroupBy` в LINQ? Какая у него сложность?**](#вопрос-6-как-работает-groupby-в-linq-какая-у-него-сложность)
- [**3. Производительность и оптимизации**](#3-производительность-и-оптимизации)
  - [**Вопрос 7: Почему `Count()` может быть медленным? Как ускорить подсчёт элементов?**](#вопрос-7-почему-count-может-быть-медленным-как-ускорить-подсчёт-элементов)
  - [**Вопрос 8: Как работает `Select` в LINQ? Какая у него сложность?**](#вопрос-8-как-работает-select-в-linq-какая-у-него-сложность)
  - [**Вопрос 9: Почему `OrderBy` может быть неэффективным? Как оптимизировать сортировку?**](#вопрос-9-почему-orderby-может-быть-неэффективным-как-оптимизировать-сортировку)
  - [**Вопрос 10: Как работает `Join` в LINQ? Какая у него сложность?**](#вопрос-10-как-работает-join-в-linq-какая-у-него-сложность)
- [**4. LINQ to Objects vs. LINQ to SQL/Entities**](#4-linq-to-objects-vs-linq-to-sqlentities)
  - [**Вопрос 11: Чем отличается выполнение LINQ to Objects от LINQ to SQL?**](#вопрос-11-чем-отличается-выполнение-linq-to-objects-от-linq-to-sql)
  - [**Вопрос 12: Как работает `AsQueryable()`? Когда его использовать?**](#вопрос-12-как-работает-asqueryable-когда-его-использовать)
  - [**Вопрос 13: Почему нельзя использовать `Count()` в LINQ to SQL без вызова `ToList()`?**](#вопрос-13-почему-нельзя-использовать-count-в-linq-to-sql-без-вызова-tolist)
- [**5. Кастомные операторы LINQ**](#5-кастомные-операторы-linq)
  - [**Вопрос 14: Как написать кастомный оператор LINQ?**](#вопрос-14-как-написать-кастомный-оператор-linq)
  - [**Вопрос 15: Как реализовать кастомный оператор `DistinctBy`?**](#вопрос-15-как-реализовать-кастомный-оператор-distinctby)
  - [**Вопрос 16: Как реализовать оператор `Batch` для пакетной обработки?**](#вопрос-16-как-реализовать-оператор-batch-для-пакетной-обработки)
- [**6. Производительность и оптимизации**](#6-производительность-и-оптимизации)
  - [**Вопрос 17: Почему `First()` быстрее `FirstOrDefault()`?**](#вопрос-17-почему-first-быстрее-firstordefault)
  - [**Вопрос 18: Как ускорить LINQ-запросы для больших коллекций?**](#вопрос-18-как-ускорить-linq-запросы-для-больших-коллекций)
  - [**Вопрос 19: Почему `ToList()` может быть опасным в LINQ to SQL?**](#вопрос-19-почему-tolist-может-быть-опасным-в-linq-to-sql)
  - [**Вопрос 20: Как работает `AsParallel()` в PLINQ? Когда его использовать?**](#вопрос-20-как-работает-asparallel-в-plinq-когда-его-использовать)

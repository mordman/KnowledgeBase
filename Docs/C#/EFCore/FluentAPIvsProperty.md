...
- [1. **Data Annotations (Атрибуты данных)**](#1-data-annotations-атрибуты-данных)
- [2. **Shadow Properties (Теневые свойства)**](#2-shadow-properties-теневые-свойства)
- [3. **Owned Entity Types (Вложенные типы сущностей)**](#3-owned-entity-types-вложенные-типы-сущностей)
- [4. **Table Splitting (Разделение таблиц)**](#4-table-splitting-разделение-таблиц)
- [5. **TPH, TPT, TPC (Стратегии наследования)**](#5-tph-tpt-tpc-стратегии-наследования)
- [6. **Global Query Filters (Глобальные фильтры запросов)**](#6-global-query-filters-глобальные-фильтры-запросов)
- [7. **Value Conversions (Конвертация значений)**](#7-value-conversions-конвертация-значений)
- [8. **Backing Fields (Поля поддержки)**](#8-backing-fields-поля-поддержки)
- [9. **Database Views (Представления базы данных)**](#9-database-views-представления-базы-данных)
- [10. **Raw SQL Queries (Сырые SQL-запросы)**](#10-raw-sql-queries-сырые-sql-запросы)
- [11. **Interceptors (Перехватчики)**](#11-interceptors-перехватчики)
- [12. **Explicit Loading (Явная загрузка)**](#12-explicit-loading-явная-загрузка)

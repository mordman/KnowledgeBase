...
## Оглавление
  - [**1. Что такое `async/await`?**](#1-что-такое-asyncawait)
  ...
  - [**3. Почему это важно?**](#3-почему-это-важно)
  - [**4. Сценарии применения**](#4-сценарии-применения)
    - [**1. Сетевые запросы**](#1-сетевые-запросы)
    - [**2. Работа с файлами**](#2-работа-с-файлами)
    - [**3. Обращение к базе данных**](#3-обращение-к-базе-данных)
    - [**4. Параллельное выполнение задач**](#4-параллельное-выполнение-задач)
    - [**5. UI-приложения (WPF, WinForms, MAUI)**](#5-ui-приложения-wpf-winforms-maui)
  - [**5. Как НЕ надо использовать `async/await`**](#5-как-не-надо-использовать-asyncawait)
  - [**6. Как работает под капотом?**](#6-как-работает-под-капотом)
  - [**7. Итоги**](#7-итоги)
    - [**1. Примеры с возвращением значений (`Task<T>`, `ValueTask<T>`)**](#1-примеры-с-возвращением-значений-taskt-valuetaskt)
      - [**1.1. Загрузка данных из API (`Task<string>`)**](#11-загрузка-данных-из-api-taskstring)
      - [**1.2. Чтение файла (`Task<string>`)**](#12-чтение-файла-taskstring)
      - [**1.3. Получение списка пользователей из БД (`Task<List<User>>`)**](#13-получение-списка-пользователей-из-бд-tasklistuser)
      - [**1.4. Вычисление суммы чисел (`Task<int>`)**](#14-вычисление-суммы-чисел-taskint)
      - [**1.5. Поиск продукта по ID (`Task<Product>`)**](#15-поиск-продукта-по-id-taskproduct)
      - [**1.6. Получение текущей погоды (`ValueTask<Weather>`)**](#16-получение-текущей-погоды-valuetaskweather)
      - [**1.7. Получение количества строк в файле (`Task<int>`)**](#17-получение-количества-строк-в-файле-taskint)
      - [**1.8. Получение случайного числа (`Task<int>`)**](#18-получение-случайного-числа-taskint)
      - [**1.9. Получение списка заказов (`Task<IEnumerable<Order>>`)**](#19-получение-списка-заказов-taskienumerableorder)
      - [**1.10. Получение хэша файла (`Task<string>`)**](#110-получение-хэша-файла-taskstring)
    - [**2. Примеры без возвращения значений (`void`, `Task`)**](#2-примеры-без-возвращения-значений-void-task)
      - [**2.1. Логирование сообщения (`Task`)**](#21-логирование-сообщения-task)
      - [**2.2. Отправка уведомления (`Task`)**](#22-отправка-уведомления-task)
      - [**2.3. Обновление данных в базе (`Task`)**](#23-обновление-данных-в-базе-task)
      - [**2.4. Очистка кэша (`Task`)**](#24-очистка-кэша-task)
      - [**2.5. Запуск фоновой задачи (`Task`)**](#25-запуск-фоновой-задачи-task)
      - [**2.6. Обработчик события в UI (`async void`)**](#26-обработчик-события-в-ui-async-void)
      - [**2.7. Запись данных в файл (`Task`)**](#27-запись-данных-в-файл-task)
      - [**2.8. Отправка данных в Kafka (`Task`)**](#28-отправка-данных-в-kafka-task)
      - [**2.9. Обработка команды (`Task`)**](#29-обработка-команды-task)
      - [**2.10. Инициализация сервиса (`Task`)**](#210-инициализация-сервиса-task)
    - [**3. Примеры с `ValueTask` (для оптимизации)**](#3-примеры-с-valuetask-для-оптимизации)
      - [**3.1. Получение кэшированных данных (`ValueTask<T>`)**](#31-получение-кэшированных-данных-valuetaskt)
      - [**3.2. Проверка существования файла (`ValueTask<bool>`)**](#32-проверка-существования-файла-valuetaskbool)
    - [**Итоги**](#итоги)
- [Сравнение **`async/await`** и **потоков (`Thread`)** в C# с точки зрения назначения, принципов работы, преимуществ и недостатков:](#сравнение-asyncawait-и-потоков-thread-в-c-с-точки-зрения-назначения-принципов-работы-преимуществ-и-недостатков)
  - [**1. Назначение**](#1-назначение)
    - [**Потоки (`Thread`)**](#потоки-thread)
    - [**`async/await`**](#asyncawait)
  - [**2. Принцип работы**](#2-принцип-работы)
    - [**Потоки (`Thread`)**](#потоки-thread)
    - [**`async/await`**](#asyncawait)
  - [**3. Преимущества**](#3-преимущества)
    - [**Потоки (`Thread`)**](#потоки-thread)
    - [**`async/await`**](#asyncawait)
  - [**4. Недостатки**](#4-недостатки)
    - [**Потоки (`Thread`)**](#потоки-thread)
    - [**`async/await`**](#asyncawait)
  - [**5. Когда что использовать?**](#5-когда-что-использовать)
  - [**6. Пример комбинирования**](#6-пример-комбинирования)
  - [**7. Вывод**](#7-вывод)
  - [Проблемы использования](#проблемы-использования)
    - [**Почему это происходит?**](#почему-это-происходит)
      - [**1. Контекст выполнения (`SynchronizationContext`)**](#1-контекст-выполнения-synchronizationcontext)
      - [**2. Пример deadlock с `Monitor` и `async`**](#2-пример-deadlock-с-monitor-и-async)
    - [**Как избежать deadlock?**](#как-избежать-deadlock)
      - [**1. Используйте `SemaphoreSlim` вместо `Monitor`**](#1-используйте-semaphoreslim-вместо-monitor)
      - [**2. Используйте `ConfigureAwait(false)`**](#2-используйте-configureawaitfalse)
      - [**3. Не смешивайте `lock` и `await`**](#3-не-смешивайте-lock-и-await)
    - [**Вывод**](#вывод)

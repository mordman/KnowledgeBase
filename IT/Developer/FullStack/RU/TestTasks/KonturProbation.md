...
dotnet Kontur.LogPacker.dll \<input\_file\> \<output\_file\>  
...
dotnet Kontur.LogPacker.dll \-d \<input\_file\> \<output\_file\>  
...
**Внимание\!** Чтобы соблюсти все базовые требования к решению, рекомендуем воспользоваться шаблоном: [https://github.com/DQKrait/Kontur.LogPacker/tree/master](https://github.com/DQKrait/Kontur.LogPacker/tree/master)  
Там также есть набор простейших тестов и утилита для запаковки решения в архив перед отправкой на проверку.

**Внимание\!** Для выполнения задания вам понадобится .NET Core SDK 2.1 и среда разработки Visual Studio 2017 или JetBrains Rider. Пробную версию JetBrains Rider можно скачать тут: [https://www.jetbrains.com/rider/](https://www.jetbrains.com/rider/)
...
* Решение должно быть запаковано в zip-архив с фиксированной структурой папок. Чтобы не ошибиться, рекомендуем воспользоваться утилитой Kontur.LogPacker.SubmitHelper из проекта с [шаблоном](https://github.com/DQKrait/Kontur.LogPacker/tree/master).

**Проверка решений**

Все решения будут проходить следующие этапы проверки:

1. Тесты, аналогичные тестам из [шаблона](https://github.com/DQKrait/Kontur.LogPacker/tree/master) решения. Если какой-то из этих тестов не пройдет, решение не будет рассматриваться дальше.  
2. Замеры степени сжатия и скорости на различных логах от реальных приложений.   
3. Код-ревью и окончательный отбор. На третий этап решения будут попадать в порядке результатов второго этапа, от лучших к худшим. Это означает, что чем лучше ваше решение относительно других, тем раньше оно будет рассмотрено и тем больше вероятность попасть в квоту мест на стажировку.  
...
...  
([файл целиком](https://github.com/DQKrait/Kontur.LogPacker/blob/master/Kontur.LogPacker.SelfCheck/example.log) можно найти в репозитории с шаблоном решения)
...
Ваш алгоритм должен хорошо работать на логах именно такого формата. Если возникнут вопросы по поводу формата (или какие-нибудь ещё вопросы), их можно задать в [чате](https://t.me/joinchat/CJx9dxKTbesr01zi_bkiyg).  
В чате также опубликован и обновляется список популярных ошибок в решениях. Загляните туда перед отправкой\!

Готовое решение [загружайте на страницу с заявкой](https://kontur.ru/education/programs/intern/backend#ApplicationForm).

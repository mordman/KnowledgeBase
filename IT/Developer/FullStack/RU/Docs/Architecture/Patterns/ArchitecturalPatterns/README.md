## 🔑 Основные архитектурные стили и паттерны



## Оглавление
- [🔑 Основные архитектурные стили и паттерны](#основные-архитектурные-стили-и-паттерны)
  - [1. **Многослойный (n-tier) паттерн**](#1-многослойный-n-tier-паттерн)
  - [2. **Чистая архитектура / Onion Architecture**](#2-чистая-архитектура-onion-architecture)
  - [3. **Объектно-ориентированный паттерн**](#3-объектно-ориентированный-паттерн)
  - [4. **Микроядро (Microkernel)**](#4-микроядро-microkernel)
  - [5. **Плагин (Plugin)**](#5-плагин-plugin)
  - [6. **Сервис-ориентированная архитектура (SOA)**](#6-сервис-ориентированная-архитектура-soa)
  - [7. **Брокер (Broker)**](#7-брокер-broker)
  - [8. **Микросервисы**](#8-микросервисы)
  - [9. **Бессерверный (Serverless / FaaS)**](#9-бессерверный-serverless-faas)
  - [10. **Пространство данных (Space-Based)**](#10-пространство-данных-space-based)
  - [11. **Одноранговая сеть (P2P)**](#11-одноранговая-сеть-p2p)
  - [12. **Шестигранная архитектура (Hexagonal / Ports & Adapters)**](#12-шестигранная-архитектура-hexagonal-ports-adapters)
  - [13. **Предметно-ориентированное проектирование (DDD)**](#13-предметно-ориентированное-проектирование-ddd)
  - [14. **Событийно-ориентированная архитектура (Event-Driven)**](#14-событийно-ориентированная-архитектура-event-driven)
  - [15. **Издатель-подписчик (Pub-Sub)**](#15-издатель-подписчик-pub-sub)
  - [16. **MVP (Model-View-Presenter)**](#16-mvp-model-view-presenter)
  - [17. **Переводчик (Interpreter)**](#17-переводчик-interpreter)
  - [18. **Оркестрация (Orchestration)**](#18-оркестрация-orchestration)
  - [19. **Хореография (Choreography)**](#19-хореография-choreography)
  - [20. **Primary-Secondary (Главный-Вспомогательный)**](#20-primary-secondary-главный-вспомогательный)
  - [21. **CQRS (Command Query Responsibility Segregation)**](#21-cqrs-command-query-responsibility-segregation)
  - [22. **Event Sourcing (Летопись событий)**](#22-event-sourcing-летопись-событий)
  - [23. **Kappa-архитектура**](#23-kappa-архитектура)
  - [24. **Lambda-архитектура**](#24-lambda-архитектура)
- [📊 Схемы в формате Mermaid](#схемы-в-формате-mermaid)
  - [1. **Многослойная архитектура (n-tier)**](#1-многослойная-архитектура-n-tier)
  - [2. **Чистая архитектура / Onion**](#2-чистая-архитектура-onion)
  - [3. **Микросервисы**](#3-микросервисы)
  - [4. **Шестигранная архитектура**](#4-шестигранная-архитектура)
  - [5. **Событийно-ориентированная архитектура (Pub-Sub)**](#5-событийно-ориентированная-архитектура-pub-sub)
  - [6. **CQRS + Event Sourcing**](#6-cqrs-event-sourcing)
  - [7. **Lambda-архитектура**](#7-lambda-архитектура)
  - [8. **Kappa-архитектура**](#8-kappa-архитектура)
  - [9. **Оркестрация vs Хореография**](#9-оркестрация-vs-хореография)
    - [Оркестрация:](#оркестрация)
    - [Хореография:](#хореография)

  - [1. **Многослойный (n-tier) паттерн**](#1-многослойный-n-tier-паттерн)
  - [2. **Чистая архитектура / Onion Architecture**](#2-чистая-архитектура-onion-architecture)
  - [3. **Объектно-ориентированный паттерн**](#3-объектно-ориентированный-паттерн)
  - [4. **Микроядро (Microkernel)**](#4-микроядро-microkernel)
  - [5. **Плагин (Plugin)**](#5-плагин-plugin)
  - [6. **Сервис-ориентированная архитектура (SOA)**](#6-сервис-ориентированная-архитектура-soa)
  - [7. **Брокер (Broker)**](#7-брокер-broker)
  - [8. **Микросервисы**](#8-микросервисы)
  - [9. **Бессерверный (Serverless / FaaS)**](#9-бессерверный-serverless-faas)
  - [10. **Пространство данных (Space-Based)**](#10-пространство-данных-space-based)
  - [11. **Одноранговая сеть (P2P)**](#11-одноранговая-сеть-p2p)
  - [12. **Шестигранная архитектура (Hexagonal / Ports & Adapters)**](#12-шестигранная-архитектура-hexagonal-ports-adapters)
  - [13. **Предметно-ориентированное проектирование (DDD)**](#13-предметно-ориентированное-проектирование-ddd)
  - [14. **Событийно-ориентированная архитектура (Event-Driven)**](#14-событийно-ориентированная-архитектура-event-driven)
  - [15. **Издатель-подписчик (Pub-Sub)**](#15-издатель-подписчик-pub-sub)
  - [16. **MVP (Model-View-Presenter)**](#16-mvp-model-view-presenter)
  - [17. **Переводчик (Interpreter)**](#17-переводчик-interpreter)
  - [18. **Оркестрация (Orchestration)**](#18-оркестрация-orchestration)
  - [19. **Хореография (Choreography)**](#19-хореография-choreography)
  - [20. **Primary-Secondary (Главный-Вспомогательный)**](#20-primary-secondary-главный-вспомогательный)
  - [21. **CQRS (Command Query Responsibility Segregation)**](#21-cqrs-command-query-responsibility-segregation)
  - [22. **Event Sourcing (Летопись событий)**](#22-event-sourcing-летопись-событий)
  - [23. **Kappa-архитектура**](#23-kappa-архитектура)
  - [24. **Lambda-архитектура**](#24-lambda-архитектура)
  - [1. **Многослойная архитектура (n-tier)**](#1-многослойная-архитектура-n-tier)
  - [2. **Чистая архитектура / Onion**](#2-чистая-архитектура-onion)
  - [3. **Микросервисы**](#3-микросервисы)
  - [4. **Шестигранная архитектура**](#4-шестигранная-архитектура)
  - [5. **Событийно-ориентированная архитектура (Pub-Sub)**](#5-событийно-ориентированная-архитектура-pub-sub)
  - [6. **CQRS + Event Sourcing**](#6-cqrs-event-sourcing)
  - [7. **Lambda-архитектура**](#7-lambda-архитектура)
  - [8. **Kappa-архитектура**](#8-kappa-архитектура)
  - [9. **Оркестрация vs Хореография**](#9-оркестрация-vs-хореография)
    - [Оркестрация:](#оркестрация)
    - [Хореография:](#хореография)
### 1. **Многослойный (n-tier) паттерн**
- Разделение приложения на слои: представление, бизнес-логика, данные.
- Часто используется в веб-приложениях.

### 2. **Чистая архитектура / Onion Architecture**
- Зависимости направлены внутрь: ядро не зависит от внешних слоёв.
- Обеспечивает тестируемость и независимость от фреймворков/БД.

### 3. **Объектно-ориентированный паттерн**
- Строится на принципах ООП: инкапсуляция, наследование, полиморфизм.

### 4. **Микроядро (Microkernel)**
- Минимальное ядро + плагины для расширения функциональности.
- Хорошо подходит для систем с изменяющимися требованиями.

### 5. **Плагин (Plugin)**
- Расширение функционала через подключаемые модули.
- Часто используется вместе с микроядром.

### 6. **Сервис-ориентированная архитектура (SOA)**
- Приложение состоит из слабосвязанных сервисов, взаимодействующих через интерфейсы.

### 7. **Брокер (Broker)**
- Посредник между компонентами, упрощает коммуникацию и маршрутизацию.

### 8. **Микросервисы**
- Каждый сервис — независимое приложение со своей БД и логикой.
- Высокая масштабируемость, но сложность в управлении.

### 9. **Бессерверный (Serverless / FaaS)**
- Логика выполняется в виде функций, запускаемых по событиям.
- Провайдер управляет инфраструктурой.

### 10. **Пространство данных (Space-Based)**
- Использует распределённое хранилище (tuple space) для обмена данными.
- Подходит для высоконагруженных систем.

### 11. **Одноранговая сеть (P2P)**
- Все узлы равноправны, нет центрального сервера.
- Пример: торрент-сети.

### 12. **Шестигранная архитектура (Hexagonal / Ports & Adapters)**
- Ядро приложения отделено от внешних систем через порты и адаптеры.
- Упрощает тестирование и замену внешних зависимостей.

### 13. **Предметно-ориентированное проектирование (DDD)**
- Фокус на предметной области, моделирование бизнес-логики.
- Часто комбинируется с CQRS, Event Sourcing.

### 14. **Событийно-ориентированная архитектура (Event-Driven)**
- Компоненты реагируют на события.
- Поддерживает асинхронность и децентрализацию.

### 15. **Издатель-подписчик (Pub-Sub)**
- Подмножество событийно-ориентированной архитектуры.
- Издатели отправляют сообщения, подписчики получают.

### 16. **MVP (Model-View-Presenter)**
- UI-паттерн для разделения логики и представления.

### 17. **Переводчик (Interpreter)**
- Паттерн для обработки языков/выражений (например, DSL).

### 18. **Оркестрация (Orchestration)**
- Централизованное управление взаимодействием сервисов.

### 19. **Хореография (Choreography)**
- Децентрализованное взаимодействие: каждый сервис реагирует на события.

### 20. **Primary-Secondary (Главный-Вспомогательный)**
- Репликация данных: один основной узел, остальные — резервные.

### 21. **CQRS (Command Query Responsibility Segregation)**
- Разделение операций записи (команд) и чтения (запросов).

### 22. **Event Sourcing (Летопись событий)**
- Состояние системы восстанавливается из последовательности событий.

### 23. **Kappa-архитектура**
- Обработка данных только через потоковую обработку (streaming).

### 24. **Lambda-архитектура**
- Гибрид потоковой и пакетной обработки данных.

---

## 📊 Схемы в формате Mermaid

### 1. **Многослойная архитектура (n-tier)**

```mermaid
graph TD
    A[UI Layer] --> B[Business Logic Layer]
    B --> C[Data Access Layer]
    C --> D[Database]
```

---

### 2. **Чистая архитектура / Onion**

```mermaid
graph TD
    Entities((Entities)) --> UseCases((Use Cases))
    UseCases --> Interfaces((Interfaces))
    Interfaces --> Frameworks((Frameworks & Drivers))
```

---

### 3. **Микросервисы**

```mermaid
graph LR
    Client --> API_Gateway
    API_Gateway --> ServiceA[Service A]
    API_Gateway --> ServiceB[Service B]
    API_Gateway --> ServiceC[Service C]
    ServiceA --> DB_A[(DB A)]
    ServiceB --> DB_B[(DB B)]
    ServiceC --> DB_C[(DB C)]
```

---

### 4. **Шестигранная архитектура**

```mermaid
graph TD
    subgraph Core
        BusinessLogic[Business Logic]
    end

    subgraph Adapters
        REST_API[REST Adapter]
        DB_Adapter[DB Adapter]
        MessageQueue[MQ Adapter]
    end

    REST_API -->|Port| BusinessLogic
    DB_Adapter -->|Port| BusinessLogic
    MessageQueue -->|Port| BusinessLogic
```

---

### 5. **Событийно-ориентированная архитектура (Pub-Sub)**

```mermaid
graph LR
    Publisher -->|Event| EventBus[(Event Bus)]
    EventBus --> Subscriber1
    EventBus --> Subscriber2
    EventBus --> Subscriber3
```

---

### 6. **CQRS + Event Sourcing**

```mermaid
graph LR
    Command --> CommandHandler
    CommandHandler --> EventStore[(Event Store)]
    EventStore --> EventHandler
    EventHandler --> ReadModel[(Read Model)]
    Client -->|Write| Command
    Client -->|Read| ReadModel
```

---

### 7. **Lambda-архитектура**

```mermaid
graph LR
    Data --> BatchLayer
    Data --> SpeedLayer
    BatchLayer --> ServingLayer
    SpeedLayer --> ServingLayer
    ServingLayer --> Query
```

---

### 8. **Kappa-архитектура**

```mermaid
graph LR
    Stream --> StreamProcessor
    StreamProcessor --> Store
    Store --> Query
```

---

### 9. **Оркестрация vs Хореография**

#### Оркестрация:

```mermaid
graph TD
    Orchestrator --> ServiceA
    Orchestrator --> ServiceB
    Orchestrator --> ServiceC
```

#### Хореография:

```mermaid
graph LR
    ServiceA -->|Event| ServiceB
    ServiceB -->|Event| ServiceC
    ServiceC -->|Event| ServiceA
```

---
...
## Оглавление
  - [Generics в C#](#generics-в-c)
- [1. Зачем нужны Generics?](#1-зачем-нужны-generics)
- [2. Примеры использования Generics](#2-примеры-использования-generics)
  - [a) Обобщённые коллекции](#a-обобщённые-коллекции)
    - [`List<T>`](#listt)
    - [`Dictionary<TKey, TValue>`](#dictionarytkey-tvalue)
    - [`HashSet<T>`](#hashsett)
  - [b) Обобщённые классы](#b-обобщённые-классы)
    - [Пример: Обобщённый класс `Box<T>`](#пример-обобщённый-класс-boxt)
  - [c) Обобщённые методы](#c-обобщённые-методы)
    - [Пример: Метод для обмена значениями](#пример-метод-для-обмена-значениями)
  - [d) Обобщённые интерфейсы](#d-обобщённые-интерфейсы)
    - [Пример: Интерфейс `IRepository<T>`](#пример-интерфейс-irepositoryt)
  - [e) Ограничения обобщений (Constraints)](#e-ограничения-обобщений-constraints)
    - [Пример: Ограничение `where T : new()`](#пример-ограничение-where-t-new)
    - [Другие ограничения:](#другие-ограничения)
- [3. Пример с кастомной коллекцией](#3-пример-с-кастомной-коллекцией)
    - [Обобщённая коллекция `MyList<T>`](#обобщённая-коллекция-mylistt)
- [4. Итоги](#4-итоги)
